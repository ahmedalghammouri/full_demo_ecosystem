 ...
 *   - the whole industry360 domain family (any subdomain, http/https, any port)
 *     so app.industry360.com, demo.industry360.sa and i360.industry360.cloud
 *     work without per-host config churn
 *
 *   `.cloud` is in that list because it is where the demonstrations actually
 *   live — lpg.industry360.cloud was already serving when i360 was deployed to
 *   the same box, and its absence here rejected every sign-in with a CORS error
 *   that named the origin but not the reason.
...
export const PUBLIC_ORIGIN =
  /^https?:\/\/([a-z0-9-]+\.)*industry360\.(com|sa|cloud)(:\d+)?$/i;
